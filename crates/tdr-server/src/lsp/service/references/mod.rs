...
use tdr_lang::db::types::{File, HirValue, HirValueKind, Project, Symbol, SymbolKind};
  ...
  fn setup_with_files(
    ...
    let mut files = HashMap::from([
      ...
      (
        ...
        File::new(
          &db,
          FileHandle::Content(root.join("content/alice.tdr"), CONTENT_ALICE.to_string()),
        ),
      ),
      (
        root.join("content/bob.tdr"),
        File::new(
          &db,
          FileHandle::Content(root.join("content/bob.tdr"), CONTENT_BOB.to_string()),
        ),
      ),
    ]);
...
"#;
    ...
    // alice.tdr, bob.tdr, file.tdr all have _type: Person, sorted by URI
    assert_eq!(
      locations.len(),
      3,
    ...
    assert!(
      ...
      "first should be alice.tdr"
    );
    assert!(
      locations[1].uri.as_str().contains("bob"),
      "second should be bob.tdr"
    );
    assert!(
      locations[2].uri.as_str().contains("file"),
      "third should be file.tdr"
    );
    ...
    assert!(
      !locations
        .iter()
        .any(|l| l.uri.as_str().contains("Person.tdr"))
    );
  }
...
"#;
    ...
    let decl = locations
      .iter()
      .find(|l| l.uri.as_str().contains("Person.tdr"))
      .expect("should include Person.tdr declaration");
  ...
  fn references_on_builtin_ident_returns_none() {
    let root = PathBuf::from(if cfg!(windows) { "C:\\vault" } else { "/vault" });
    let schema_path = root.join("schemas/Person.tdr");
    let schema_raw = r#"---
_type: sc|hema
properties:
  name:
...
"#;
    ...
    let fref_location = locations
      .iter()
      .find(|l| l.uri == uri)
      .expect("should include file.tdr (the fref site)");
    // the fref call is on line 2
    // alice.tdr starts right after the opening quote at col 12, ends at col 21
    ...
    assert_eq!(
      fref_location.range.end.character, 21,
      "path string ends at col 21"
    );
  }
...
"#,
    ...
    let params = make_params(uri.clone(), &content, offset, false);
    let locations = find_references(&analysis, params).expect("should find references");
    // Find the location in file.tdr itself
    let own_location = locations
      .iter()
      .find(|l| l.uri == uri)
      .expect("should have a location in file.tdr");
