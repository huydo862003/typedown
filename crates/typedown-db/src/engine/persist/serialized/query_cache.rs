...
/// A footer entry: maps a dep graph node index to the byte offset of its cached result.
#[derive(Debug, Clone, Copy)]
...
pub const CACHE_ENTRY_SIZE: usize = 12; // 4 + 8
...
/// Previous session's query cache, backed by mmap'd file data.
...
pub struct SerializedQueryCache {
  /// Raw pointer to the mmap'd file data.
  ptr: *const u8,
  /// Length of the mmap'd region in bytes.
  len: usize,
  /// Index: dep graph node index -> byte offset of the result blob.
  index: HashMap<u32, u64>,
}

// Safety: the mmap'd data is read-only and shared
unsafe impl Send for SerializedQueryCache {}
unsafe impl Sync for SerializedQueryCache {}

impl SerializedQueryCache {
  /// Create from a raw pointer to mmap'd file data.
  /// Decodes the footer index upfront.
  ///
  /// # Safety
  /// `ptr` must point to a valid memory region of `len` bytes that remains
  ...
  pub unsafe fn from_raw(ptr: *const u8, len: usize) -> Option<Self> {
    ...
    Some(SerializedQueryCache { ptr, len, index })
  }
